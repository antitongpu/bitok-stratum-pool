...
class StratumServer extends events.EventEmitter {
    ...
    init() {
        if (this.options.banning && this.options.banning.enabled) {
            setInterval(() => {
                ...
                for (const ip in this.malformedCounts) {
                    if (Date.now() - this.malformedCounts[ip].first > 5 * 60 * 1000) {
                        delete this.malformedCounts[ip];
                    }
                }
        ...
        setInterval(() => {
            for (const subId in this.stratumClients) {
                const client = this.stratumClients[subId];
                if (client.socket.destroyed) {
                    this.removeStratumClientBySubId(subId);
    ...
    handleNewClient(socket) {
        ...
        client.on('socketDisconnect', () => {
            ...
            if (!this.malformedCounts[ip]) {
                this.malformedCounts[ip] = { count: 0, first: Date.now() };
            }
            this.malformedCounts[ip].count++;
            if (this.malformedCounts[ip].count >= 3) {
                const count = this.malformedCounts[ip].count;
                this.addBannedIP(ip);
                delete this.malformedCounts[ip];
                this.emit('bannedForMalformed', ip, count);
            }
        });
